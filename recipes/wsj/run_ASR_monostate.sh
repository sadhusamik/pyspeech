...
## ASR with monophone and monostate HMMs
...
exp_dir=exp 
data_dir=data
feat_suff=      # Suffix for same feature but with multiple 
...
if [ $stage -le 1 ]; then
  add_opts="--add_reverb=large_room --set_unity_gain"
  for x in test_eval92 train_si284; do
...
if [ $stage -le 2 ]; then
 add_opts="--add_reverb=large_room --set_unity_gain" 
  for x in test_eval92 train_si284; do
    local_pyspeech/make_modspec_mfcc_feats.sh --nj $nj_modspec \
...
if [ $stage -le 3 ]; then
  ...
  # Change the topo file for monostate HMMs
  cp ./conf/topo_single_state_hmm ./data/lang_nosp/topo
 ...
 if $train; then
  ...
  # Fetch the modspec features for getting hybrid model
    ...
    steps/nnet2/train_tanh_fast.sh \
      ...
      data/train_si284 data/lang_nosp $exp_dir/mono0a_ali \
      $exp_dir/hybrid_modspecMfcc || exit 1
  fi
 ...
 if $train; then
  ...
  # Fetch the modspec features for getting hybrid model
    ...
    steps/nnet2/train_tanh_fast.sh \
      ...
      --num-hidden-layers 5  \
      --num_threads 1 \
      --parallel_opts "-l 'hostname=b1[12345678]*|c*' --gpu 1"\
      --hidden_layer_dim 256 \
      --num-jobs-nnet 16 \
      --splice_width 0 \
      --feat_type "raw" \
